...
@group(0) @binding(12) var<uniform> contact_shadows: types::ContactShadows;

const VISIBILITY_RANGE_UNIFORM_BUFFER_SIZE: u32 = 64u;
#if AVAILABLE_STORAGE_BUFFER_BINDINGS >= 6
@group(0) @binding(13) var<storage> visibility_ranges: array<vec4<f32>>;
#else
@group(0) @binding(13) var<uniform> visibility_ranges: array<vec4<f32>, VISIBILITY_RANGE_UNIFORM_BUFFER_SIZE>;
#endif

@group(0) @binding(14) var<uniform> ssr_settings: types::ScreenSpaceReflectionsSettings;
...
// NB: If you change these, make sure to update
// `TONEMAPPING_LUT_TEXTURE_BINDING_INDEX` and
// `TONEMAPPING_LUT_SAMPLER_BINDING_INDEX` in `bevy_pbr/src/render/mesh.rs`!
...
#ifdef MULTISAMPLED
@group(0) @binding(29) var single_sampled_depth_prepass_texture: texture_depth_2d;
#endif  // MULTISAMPLED

// Allows us to sample from the depth buffer with bilinear filtering.
@group(0) @binding(30) var depth_linear_sampler: sampler;
// Allows us to sample from the depth buffer with nearest-neighbor filtering.
@group(0) @binding(31) var depth_nearest_sampler: sampler;
