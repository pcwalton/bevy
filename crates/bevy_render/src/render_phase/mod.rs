    ...
    BPI: BinnedPhaseItem,
{
    ...
    /*
    pub fn remove(&self, main_entity: MainEntity) {
        let Some(cached_binned_entity) = self.cached_entity_bin_keys.remove(&main_entity) else {
            return;
        };
        let Some(cached_bin_key) = cached_binned_entity.cached_bin_key else {
            return;
        };
        match cached_bin_key.phase_type {
            BinnedRenderPhaseType::MultidrawableMesh => {
                if let Some(mut bin) = self
                    .multidrawable_meshes
                    .get_mut(&cached_bin_key.batch_set_key)
                    .and_then(|bins| bins.get_mut(&cached_bin_key.bin_key))
                {
            ...
            BinnedRenderPhaseType::BatchableMesh => {
                if let Some(mut bin) = self
                    .batchable_meshes
                    .get_mut(&(cached_bin_key.batch_set_key, cached_bin_key.bin_key))
                {
                    bin.remove(main_entity);
                }
            }
            BinnedRenderPhaseType::UnbatchableMesh => {
                if let Some(mut bin) = self
                    ...
                    .get_mut(&(cached_bin_key.batch_set_key, cached_bin_key.bin_key))
                {
                    // FIXME: I think we have to do more here
                    bin.entities.remove(&main_entity);
                }
            }
            BinnedRenderPhaseType::NonMesh => {
                if let Some(mut bin) = self
    ...
    I: SortedPhaseItem,
{
    ...
    #[inline]
    pub fn add(&mut self, main_entity: MainEntity, item: I) {
        self.items.insert(main_entity, item);
    }
