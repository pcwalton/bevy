...
use crate::{
    mesh::{MeshVertexBufferLayout, MissingVertexAttributeError},
    render_resource::{
...
pub struct SpecializedMeshPipelines<S: SpecializedMeshPipeline> {
    mesh_layout_cache: HashMap<u64, HashMap<S::Key, CachedRenderPipelineId>>,
...
impl<S: SpecializedMeshPipeline> SpecializedMeshPipelines<S> {
    ...
    pub fn specialize(
        ...
        let map = self.mesh_layout_cache.entry(layout.hash()).or_default();
        match map.entry(key.clone()) {
            Entry::Occupied(entry) => return Ok(*entry.into_mut()),
            Entry::Vacant(entry) => {
                return specialize_slow(
        ...
        fn specialize_slow<S: SpecializedMeshPipeline>(
            vertex_layout_cache: &mut VertexLayoutCache<S>,
            entry: VacantEntry<S::Key, CachedRenderPipelineId>,
            cache: &PipelineCache,
            specialize_pipeline: &S,
            layout: &MeshVertexBufferLayout,
        ) -> Result<CachedRenderPipelineId, SpecializedMeshPipelineError> {
