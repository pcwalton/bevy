...
/// Data that uniquely identifies a compiled shader pipeline.
...
/// GPU data specific to a single gradient quad that's constant across the quad.
#[repr(C)]
...
/// Computes the set of stop indices that have nonzero distance between them.
///
/// Stop indices not in this list aren't sent to the GPU at all.
