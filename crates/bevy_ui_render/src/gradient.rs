...
impl UiPrepareRenderObject for ExtractedGradient {
    ...
    fn populate_quad(
        ...
        let stop_index = self.rendered_stop_indices[index] as usize;
        let mut start_stop = self.stops[stop_index];
        let end_stop = self.stops[stop_index + 1];
        if start_stop.1 == end_stop.1 && stop_index == self.stops.len() - 2 && 0 < index {
        ...
        if 0. < start_stop.1 && (stop_index == 0 || index == 0) {
