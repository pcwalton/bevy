...
/// GPU data specific to a single texture slice quad that's constant across the
/// quad.
#[repr(C)]
...
pub struct UiTextureSliceInstanceData {
    ...
    ///
    /// If bindless textures aren't being used, this is 0.
...
pub struct UiTextureSlicePipeline {
    ...
    /// The layout of the textured bind group when bindless is in use.
    ///
    /// If bindless isn't in use, this is an empty descriptor.
    pub textured_bindless_layout: BindGroupLayoutDescriptor,
    /// The layout of the textured bind group when non-bindless is in use.
    pub textured_non_bindless_layout: BindGroupLayoutDescriptor,
    pub shader: Handle<Shader>,
}

impl UiTextureSlicePipeline {
    /// Returns the layout of the textured bind group for the texture slice UI
...
pub fn extract_ui_texture_slices(
    ...
    mut textured_image_bind_groups: ResMut<UiTexturedBindGroups<GpuImage>>,
