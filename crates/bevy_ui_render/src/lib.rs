...
use bevy_math::{proj, vec4, Affine2, FloatOrd, Rect, UVec4, Vec2, Vec4};
use bevy_render::{
...
/// Data specific to a single UI node quad that's constant across the quad.
...
impl UiRenderObject for ExtractedUiNode {
    ...
    fn populate_quad(
        ...
        for (&mut (ref mut out_position, ref mut out_interpolants), (position, (uv, point))) in
            out_quad
                .vertices
                .iter_mut()
...
where
    E: UiRenderObject,
{
    /// The GPU vertex buffer needed to render the objects of this type.
...
where
    E: UiRenderObject,
{
    /// The range of indices within the [`UiMeta::indices`] for this batch.
    pub range: Range<u32>,
